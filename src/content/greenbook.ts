...
const greenbookContentZh = {
  ...
  chapters: [
    ...
    },
    {
      ...
      title: "900 亿赛季规则池",
      eyebrow: "03 / 主引擎",
      summary: "900 亿是赛季机制中的规则库存说明：最多 10 个赛季 x 18 轮 x 每轮 5 亿。",
      defaultOpen: true,
      paragraphs: [
        "72H 的经济模型，最核心的数字只有一个：90,000,000,000 72H。900 亿。该数字是规则说明，不代表任何地址当前可领取或获得收益。",
    ...
    },
    {
      ...
      title: "从用户到建设者",
      eyebrow: "08 / 长期路径",
      summary: "72H 更长期的一点，不是 900 亿赛季规则池，而是它没有把用户终点设成继续买币。",
      defaultOpen: false,
      paragraphs: [
  ...
  economyTable: {
    title: "90% 给赛季，故事才有重量",
    summary: "72H 总供应固定为 100,000,000,000 72H。900 亿被规划为赛季规则池；成功轮次不是立刻、自动、无条件发放；当前不代表任何地址可领取，而是在赛季结算、名单发布、领取窗口和机制阈值等条件满足后进入分批领取。",
    allocationTitle: "总供应分配",
    roundTitle: "每轮 5 亿四条战线",
    allocations: [
      { name: "赛季玩法池", amount: "90,000,000,000 72H", share: "90%", role: "72H 的核心赛季储备" },
      { name: "早期预约 / 后续发行池", amount: "4,500,000,000 72H", share: "4.5%", role: "早期预约与后续规则核对，开放安排以正式公告为准" },
    ...
    roundAllocations: [
      ...
      { lane: "排行榜", share: "10%", amount: "50,000,000 72H", meaning: "你是否在公开竞争里形成可核验记录" },
    ],
  },
  claimUnlock: {
    title: "成功轮次的领取机制说明（未开放）",
    ...
    scheduleTitle: "按机制阈值示例（未开放）",
    scheduleNotice: "以下价格仅为机制阈值示例，当前未开放、不是报价、不是价格预测、不是价格目标、不是上市安排、不是购买或收益承诺。",
    schedule: [
      { price: "$0.01", unlock: "累计 20%", detail: "达到该机制阈值并连续满足 72 小时后，规则上限示例为已分配额度的 20%" },
      { price: "$0.03", unlock: "累计 40%", detail: "第二档机制阈值连续满足 72 小时后，规则上限示例累计 40%" },
      { price: "$0.05", unlock: "累计 60%", detail: "第三档机制阈值连续满足 72 小时后，规则上限示例累计 60%" },
      { price: "$0.07", unlock: "累计 80%", detail: "第四档机制阈值连续满足 72 小时后，规则上限示例累计 80%" },
      { price: "$0.10", unlock: "累计 100%", detail: "第五档机制阈值连续满足 72 小时后，规则上限示例累计 100%" },
    ],
    note: "本节仅解释机制，当前未开放领取，不代表任何地址可领取，也不代表任何地址一定获得分配。价格档位仅为领取机制中的阶段阈值，不构成价格目标、价格预测、上市安排或收益承诺。实际可领取额度取决于官方名单、有效证明、领取窗口、链上状态、已领取记录和网络手续费等条件。",
  },
  pathSteps: {
    title: "从看见到参与：72H 行动路径",
    ...
    steps: [
      ...
      { title: "加入战队", kicker: "03", body: "理解战队规则。战队把个人行动变成可核验协作记录，帮助普通用户看清自己在规则中的位置。" },
      { title: "查看排行榜规则", kicker: "04", body: "排行榜用于展示公开记录。你不再只是一个地址，而是可以看到轮次、战队、邀请、记录和名次如何被核验。" },
    ],
  },
  closing: {
    title: "先理解规则，再决定是否参与。",
    body: "72H 把固定供应、900 亿赛季规则池、72 小时轮次、冲刺通行证、战队协作、WAN 服务入口、赛季战场和建设者路径放在同一张公开地图上。真正重要的不是一句口号，而是用户能否看懂规则、判断风险，并用行动留下记录。",
  ...
  share: {
    title: "先理解规则，再决定是否参与",
    subtitle: "规则说明版。72 小时窗口。从用户到建设者。",
    bullets: ["先理解规则，再决定是否参与", "900 亿赛季规则池", "72 小时冲刺", "从用户到建设者"],
...
const greenbookContentEn = {
  ...
  chapters: [
    ...
    },
    {
      ...
      summary: "90B is a rules-inventory explanation for the season mechanism: up to 10 seasons x 18 rounds x 500M per round.",
      defaultOpen: true,
      paragraphs: [
        "The core number in the 72H economic model is 90,000,000,000 72H. 90B. This is a rules explanation, not a current claim route or return promise.",
  ...
  economyTable: {
    ...
    summary: "Total supply is fixed at 100,000,000,000 72H. 90B is planned as the season rules pool. Successful rounds are not automatic, live, or unconditional distributions; they can enter staged claim only after season finalization, list publication, an active claim window, and mechanism-threshold conditions.",
    ...
    allocations: [
      ...
      { name: "Early reservation / later issuance pool", amount: "4,500,000,000 72H", share: "4.5%", role: "Early reservation and later rule review; any opening depends on official announcements" },
  ...
  claimUnlock: {
    title: "Successful-round claim mechanism (not live)",
    summary: "This section explains the mechanism only. Claims are not currently open; it does not mean any address can claim or that any address is guaranteed an allocation. Each 500,000,000 72H round can enter the claim path only after the round succeeds and later confirmations are complete; eligible users may claim in stages only after the official list and claim window are published.",
    ...
    scheduleTitle: "Mechanism-threshold examples (not live)",
    scheduleNotice: "The price levels below are mechanism-threshold examples only. They are not live, not quotes, not price targets, not forecasts, not listing plans, and not purchase or return promises.",
    schedule: [
      { price: "$0.01", unlock: "20% cumulative", detail: "If this mechanism threshold is continuously satisfied for 72 hours, the rule example caps staged availability at 20% of the allocation" },
      { price: "$0.03", unlock: "40% cumulative", detail: "If the second mechanism threshold is continuously satisfied for 72 hours, the rule example caps cumulative staged availability at 40%" },
      { price: "$0.05", unlock: "60% cumulative", detail: "If the third mechanism threshold is continuously satisfied for 72 hours, the rule example caps cumulative staged availability at 60%" },
      { price: "$0.07", unlock: "80% cumulative", detail: "If the fourth mechanism threshold is continuously satisfied for 72 hours, the rule example caps cumulative staged availability at 80%" },
      { price: "$0.10", unlock: "100% cumulative", detail: "If the fifth mechanism threshold is continuously satisfied for 72 hours, the rule example caps cumulative staged availability at 100%" },
    ],
    note: "This section explains the mechanism only. Claims are not currently open; it does not mean any address can claim, that any address is guaranteed an allocation, or that any price level will be reached. Price levels are mechanism thresholds for staged claim availability only, not price targets, forecasts, listing plans, or return promises. Actual claimable amounts depend on official lists, valid proofs, active windows, on-chain state, prior claims, and network fees.",
  },
  pathSteps: {
    title: "From Watching to Participation",
    summary: "This is not a promise of outcomes. It is a participation map: understand the rules, enter a round, complete valid actions, join a squad, and build a public record.",
    steps: [
      ...
      { title: "Join a squad", kicker: "03", body: "Understand squad rules. Squads turn individual actions into verifiable coordination records and help users see their position in the mechanism." },
      { title: "Review leaderboard rules", kicker: "04", body: "The leaderboard displays public records. You are no longer only an address; you can see how rounds, squads, invites, records, and ranks are reviewed." },
    ],
  },
  closing: {
    title: "Understand the rules. Decide your role.",
    body: "72H brings fixed supply, a 90B season rules pool, 72-hour rounds, Rush Pass, squads, WAN, Season War, and the builder path into one public map. The point is not a slogan; it is whether users can understand the rules, evaluate the risks, and leave verifiable records through action.",
  ...
  share: {
    title: "Understand the Rules. Decide Your Role.",
    subtitle: "Rules summary. 72-hour window. From user to builder.",
    bullets: ["Understand the Rules. Decide Your Role.", "90B season rules pool", "72-hour sprint", "From user to builder"],
