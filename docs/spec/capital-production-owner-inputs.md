...
This file lists the exact owner-supplied inputs required before `npm run capital:gate:production` can pass. Do not paste secrets into git or chat. Store secrets only in the production Cloudflare project, the production database provider, or a local ignored `.env.production.local` used for one-time gate execution.
...
- `DATABASE_URL`: production Postgres connection string after migrations and seed.
...
| Field | How to get it |
| --- | --- |
| `TON_MAINNET_72H_JETTON_MASTER_ADDRESS` | Verify the official 72H Jetton master. Must be `EQDvE0ffdwvOhILjRJKFd2bIU9t5H9bG3-SKRidqavZjRsw8`. |
...
Use the templates in `docs/spec/capital-production-artifacts/` and replace them with real signed evidence.
